...
async function loading(dest, zk) {
  const lod = [
    ...
    "⬜⬜⬜⬜⬛⬛⬛⬛80%",
    "⬛⬛⬜⬜⬜⬜⬛⬛100%",
    "*ᴛᴏғᴀɴ-ᴍᴅ sᴘᴇᴇᴅ ᴛᴇsᴛ ᴏʀɪɢɪɴᴀᴛᴇᴅ ғʀᴏᴍ ᴛʜᴇ sᴀᴠᴇʀ*"
  ];

  let { key } = await zk.sendMessage(dest, { text: 'Tofan md speed test Loading!!! Please Wait' });
  ...
  const audioMessage = {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        sourceUrl:  'https://whatsapp.com/channel/0029Vaw6yRaBPzjZPtVtA80A' , // Corrected variable name
        mediaType: 1,
...
keith({
  ...
  try {
    ...
    await repondre("> *ᴛᴏғᴀɴ-ᴍᴅ is Restarting from the server...*");
...
keith({
  ...
  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or ᴛᴏғᴀɴ-ᴍᴅh*");
  }
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          sourceUrl:  'https://whatsapp.com/channel/0029Vaw6yRaBPzjZPtVtA80A' , // Your channel URL
          mediaType: 1,
